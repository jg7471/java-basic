package etc.Z3etc.generic.basic;

public class MainClass {

    //제네릭(Generic)은 클래스 내부에서 지정하는 것이 아닌 외부에서 사용자에 의해 지정되는 것을 의미
    /*
         # 제네릭
         - 클래스 내부에서 타입을 지정하는 것이 아니라
          외부에서 객체를 생성할 때 타입이 지정될 수 있게 하는 문법.
          제네릭을 사용하면 객체로 생성할 때 사용할 타입을 지정할 수 있고
          따로 타입을 체크하고 변환해줄 필요가 없습니다.
          관리가 용이하고 잘못된 타입이 들어올 수 있는 것을 방지하기 때문에
          유지보수성, 코드의 재사용성이 높아집니다.
         */

    public static void main(String[] args) {

        Box<String> box = new Box<>(); //1선언 // String <>한번만 ok
        box.setObj(new String("홍길동")); //2set //String 타입으로 바뀜
        int length = box.getObj().length(); //3get //getobj 리턴 타입 스트링으로 바뀌어서 length 가능 : 형변환X


        Box<Integer> intBox = new Box<>();
        intBox.setObj(20);//인티져 타입으로 됨
        Integer num = intBox.getObj(); // 지역변수

        Box<Person>pBox = new Box<>();
        pBox.setObj(new Person());//펄슨타입
        String name = pBox.getObj().getName();//리턴 펄슨타입


    }
}
