...
public class InputStreamEx {
    ...
    public static void main(String[] args) {
        ...
        //try-with-resources 사용하려면 close 하려는 객체가 AutoCloseable 인터페이스와 관계여야 합니다 (2번) //마우스 왼클릭해서 Closeable 속성 있는지 체크
        try(FileInputStream fis = new FileInputStream("C:\\Users\\ICT4_15\\Desktop\\file\\JavaIO\\test.txt");/*2번*/) {
            //fis = new FileInputStream("C:/MyWorkspace/test.txt");
            int data = 0; //data = 0 초기화
            while(data != -1){//메서드가 더 이상 읽어올 값이 없다면 -1 반환
            //while(true){ //원본
                data = fis.read(); // 1바이트 단위로 데이터를 읽어들임 //read(): 입력 스트림으로부터 1바이트를 읽고 읽은 바이트를 리턴.
