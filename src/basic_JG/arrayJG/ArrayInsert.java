...
public class ArrayInsert {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            //if(name == "그만"){//자바에서는 요렇게 X
            if(name.equals("그만")){//자바 Style 문자열 only
        ...
        //System.out.println(Arrays.toString(names)); //모든 값(null포함 출력)
        System.out.println("-------------------");
        for(String n : names){
            if(n == null) break;
            System.out.print(n + " "); //null 미출력
        }
        sc.close();
    }
}