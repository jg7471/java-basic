...
public class A3FieldAndLocal {
    ...
    */
    ...
    void printNumber(int c){ // 매개변수(지역변수)
        //int b; // 지역 변수 : 반드시 초기화를 수동으로 해야함
        int b = 10; // 지역 변수
        System.out.println("필드 a = " + a);
        System.out.println("지역변수 b = " + b); //초기화 안되어 있으면 사용불가
        System.out.println("매개변수 c = " + c);
    }
    void foo(){
        System.out.println("필드 a = " + a); // 필드는 어느 메서드에서든 참조 가능(같은 메서드면 사용가능)
        //System.out.println("지역변수 b = " + b); //지역변수라 사용불가
    }
}