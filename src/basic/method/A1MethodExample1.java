...
public class A1MethodExample1 {
    // 메서드의 선언!(static은 뒤에서 알려드릴게요)
    static int calcTotal(int end){//static int calcTotal 리턴타입 명시 //매개변수 = 타입(해당값) 변수 설정
        int total = 0;
        for(int i = 1; i <= end; i++){
            total += i;
        }
        return total;//리턴값도 int
        
    }
    public static void main(String[] args) {
        int result = calcTotal(10);//리턴되는 type int
